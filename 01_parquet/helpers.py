...
def find_type_conflicts(jsonl_path):
    """
    Streams a JSONL file to find fields that have inconsistent types across different lines,
    reporting each unique conflict only once and ignoring safe conflicts (e.g., int/float).

    Args:
        jsonl_path (str or Path): The path to the JSONL file.
    """
    ...
    def check_record(obj, path, line_num):
        ...
        # Check for conflicts if a type has been seen for this path before
        if path in field_types:
            expected_type = field_types[path]

            # Define what constitutes a conflict we care about
            is_a_conflict = False
            if expected_type != current_type:
                # It's a conflict unless it's a safe mix of int and float
                is_safe_numeric_mix = ({expected_type, current_type} == {'int', 'float'})
                if not is_safe_numeric_mix:
                    is_a_conflict = True

            if is_a_conflict and path not in reported_conflicts:
                print(f"❌ Conflict first found on line {line_num} at path '{path}':")
                print(f"   - Expected type '{expected_type}' but got '{current_type}'")
                print(f"   - Conflicting Value: {repr(obj)}")
                reported_conflicts.add(path)
                conflicts_found = True
        else:
            # First time seeing this path, record its type
            field_types[path] = current_type

        # If we see a float where we previously saw an int, upgrade the type
        if path in field_types and current_type == 'float' and field_types[path] == 'int':
            field_types[path] = 'float'

        # --- RECURSION ---
        # Recurse into nested structures
        if isinstance(obj, dict):
            for k, v in obj.items():
                check_record(v, f"{path}.{k}" if path else k, line_num)
        elif isinstance(obj, list):
            # Recursively check items within the list
            for item in obj:
                check_record(item, f"{path}[]", line_num)

    print(f"🔍 Starting type analysis for: {jsonl_path}\n")
    with open(jsonl_path, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f, 1):
            try:
                ...
                print(f"⚠️ Warning: Could not decode JSON on line {i}")

    print("\n" + "="*50)
    if not conflicts_found:
        print("✅ Analysis complete. No type conflicts found!")
    else:
        print("⚠️ Analysis complete. Unique conflicts were found. Please review the output above.")
